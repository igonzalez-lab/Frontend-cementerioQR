...
const items = [{
    value: 'tradiciones_del_pesaj',
    item: 'Tradiciones del Pesaj (Pascua judía)'
}, {
    value: 'costumbres_de_rosh_hashana_y_yom_kipur',
    item: 'Costumbres de Rosh Hashaná y Yom Kipur'
}, {
    value: 'celebraciones_de_sucot_januca_y_purim',
    item: 'Celebración de Sucot, Janucá y Purim'
}, {
    value: 'recetas_familiares_especiales',
    item: 'Recetas familiares especiales'
}, {
    value: 'canciones_y_melodias_tradicionales',
    item: 'Canciones y melodías tradicionales'
}, {
    value: 'fotos_historicas_de_celebraciones',
    item: 'Fotos históricas de celebraciones'
}];
...
const Festividades = ({ people, viewOnly }) => {
    ...
    const handleGetData = useCallback(async (page, rows, id) => {
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_BACKEND_ADDRESS}/api-ajch/traditions/${page}/${rows}/${id}/SI`, requestOptions);
    ...
    }, [token, setData, dispatch, setDataError, setDataErrorMessage]);
    ...
    const handleGetDataCount = useCallback(async (id) => {
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_BACKEND_ADDRESS}/api-ajch/traditions/count/${id}/SI`, requestOptions);
            const responseJson = await response.json();
    ...
    const handleGetDataSearch = useCallback(async (value, id) => {
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_BACKEND_ADDRESS}/api-ajch/traditions/param/${value}/${id}/SI`, requestOptions);
            const responseJson = await response.json();
    ...
    const handlePostData = useCallback(async () => {
        try {
            ...
            const requestBody = JSON.stringify({
                ...
                'festividad': 'SI',
                'tradition_type': tipoActividad,
                'tradition_description': JSON.stringify(descripcionActividad)
            });
    ...
    }, [people, token, page, rowsPerPage, filterValue, tipoActividad, descripcionActividad, dispatch, setLoading, handleGetData, setTipoActividad, setDialogCreate, handleGetDataSearch, setDescripcionActividad]);
    ...
    const handlePutData = useCallback(async () => {
        try {
            ...
            const requestBody = JSON.stringify({
                'people_id': String(people.id),
                'tradition_id': String(selectedData.id),
                'tradition_type': tipoActividad,
                'tradition_description': JSON.stringify(descripcionActividad)
            });
    ...
    }, [people, token, page, rowsPerPage, filterValue, selectedData, tipoActividad, descripcionActividad, dispatch, setLoading, handleGetData, setTipoActividad, setDialogUpdate, handleGetDataSearch, setDescripcionActividad]);
    ...
    }, [people, token, page, rowsPerPage, filterValue, selectedData, dispatch, handleGetData, setDialogDelete, handleGetDataSearch]);
    ...
    const handleOpenDialogCreate = useCallback(() => {
        ...
        setDescripcionActividad({});
    }, [setDialogCreate, setDescripcionActividad]);

    const handleOpenDialogUpdate = useCallback(async (payload) => {
        setDialogUpdate(true);
        setSelectedData(payload);
        setTipoActividad(payload.tipo_tradicion);
        setDescripcionActividad(JSON.parse(payload.descripcion_tradicion));
    ...
    return (
        <Fragment>
                ...
                {!viewOnly && (
                    <Fragment>
                        <Button disableElevation color='primary' variant='contained' startIcon={<Add />} sx={{ mr: 2, mb: 2 }} onClick={handleOpenDialogCreate}>
                            <Typography variant='button'>Crear Festividad</Typography>
                        </Button>
                        ...
                        <Button disableElevation color='primary' variant='contained' startIcon={<Refresh />} sx={{ mr: 2, mb: 2 }} onClick={() => { filterValue !== '' ? handleGetDataSearch(filterValue, people.id) : handleGetData(page, rowsPerPage, people.id) }}>
                            <Typography variant='button'>Recargar Festividades</Typography>
                        </Button>
                    </Fragment>
                )}
                            ...
                            <Box component={Paper} elevation={0} variant='outlined' overflow={'auto'} m={'auto'}>
                                <TableContainer sx={{ maxHeight: '53vh' }}>
                                    <Table size='small' stickyHeader>
                                        <TableHead>
                                            <TableRow>
                                                <TableCell align='center' sx={{ fontSize: 17 }}>Tipo de Festividad</TableCell>
                                                <TableCell />
                                                {!viewOnly && (
                                                    <Fragment>
                                                        <TableCell />
                                        ...
                                        <TableBody>
                                            {dataSearch.map((value, index) => (
                                                <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 }, }}>
                                                    <TableCell align='center'>{items.filter(x => String(x.value) === String(value.tipo_tradicion))[0].item}</TableCell>
            ...
            <Dialog open={dialogCreate} onClose={() => setDialogCreate(false)} maxWidth='md' fullScreen={fullScreen}>
                <DialogTitle>Crear Nueva Festividad</DialogTitle>

                <DialogContent>
                    <Box component={Paper} elevation={0} variant='outlined' bgcolor={grey['300']} p={2} display={'flex'} alignItems={'center'} mb={2}>
                        <Info color='action' />
                        <Typography color='textSecondary' ml={2}>Completa los campos para ingresar la festividad, los detalles de esta, deberás ingresarlas en el editor de texto enriquecido.</Typography>
                    </Box>
            ...
            <Dialog open={dialogUpdate} onClose={() => setDialogUpdate(false)} maxWidth='md' fullScreen={fullScreen}>
                <DialogTitle>Editar Festividad</DialogTitle>

                {selectedData === null ? (
                    <Box component={Paper} elevation={0} variant='outlined' display={'flex'} alignItems={'center'} p={2}>
                        <Warning fontSize='large' color='warning' />
                        <Typography ml={2}>Seleccione una festividad para continuar</Typography>
                    </Box>
                ) : (
                    <Fragment>
                        <DialogContent>
                            <Box component={Paper} elevation={0} variant='outlined' bgcolor={grey['300']} p={2} display={'flex'} alignItems={'center'} mb={2}>
                                <Info color='action' />
                                <Typography color='textSecondary' ml={2}>Completa los campos para editar la festividad, los detalles de esta, deberás ingresarlas en el editor de texto enriquecido.</Typography>
                            </Box>
            ...
            <Dialog open={dialogDelete} onClose={() => setDialogDelete(false)} maxWidth='md' fullScreen={fullScreen}>
                <DialogTitle>Eliminar Festividad</DialogTitle>
                <DialogContent>
                    {selectedData === null ? (
                        <Box component={Paper} elevation={0} variant='outlined' display={'flex'} alignItems={'center'} p={2}>
                            <Warning fontSize='large' color='warning' />
                        ...
                        <Fragment>
                            <Typography textAlign={'justify'}>¿Esta seguro de eliminar la festividad <b>{items.filter(x => String(x.value) === String(selectedData.tipo_tradicion))[0].item}</b> del sistema?, una vez eliminada deberá crearla nuevamente si desea restablecerla.</Typography>
            ...
            <Dialog open={dialogDetail} onClose={() => setDialogDetail(false)} maxWidth='md' fullScreen={fullScreen}>
                <DialogTitle>Detalles de la Festividad</DialogTitle>
                <DialogContent>
                    {selectedData === null ? (
                        <Box component={Paper} elevation={0} variant='outlined' display={'flex'} alignItems={'center'} p={2}>
                            <Warning fontSize='large' color='warning' />
                                    ...
                                    <Table>
                                        <TableBody>
                                            <TableRow sx={{ '&:last-child td, &:last-child th': { border: 0 }, }}>
                                                <TableCell sx={{ fontWeight: 900, bgcolor: grey['100'] }}>Tipo de Festrividad</TableCell>
                                                <TableCell>{items.filter(x => String(x.value) === String(selectedData.tipo_tradicion))[0].item}</TableCell>
                                            </TableRow>
                                        </TableBody>
                                    </Table>
                                </TableContainer>
                            ...
                            <Box component={Paper} elevation={0} variant='outlined' overflow={'auto'} m={'auto'} mt={2}>
                                <TableContainer>
                                    <Table>
                                        <TableBody>
                                            <TableRow>
                                                <TableCell sx={{ fontWeight: 900, bgcolor: grey['100'] }}>Descripción de la Festividad</TableCell>
                                            </TableRow>
                                            <TableRow sx={{ '&:last-child td, &:last-child th': { border: 0 }, }}>
                                                <ReactQuill
                                                    value={JSON.parse(selectedData.descripcion_tradicion)}
                                                    theme={null}
                                                    className='rounded'
                                                    readOnly
                                                />
