...
export default function Index({ people, viewOnly, personal }) {
    ...
    const [selectedYear, setSelectedYear] = useState(() => {
        if (data !== null) {
            const years = data.map((x) => parseInt(x.ano));
    ...
    const yearRecords = useMemo(() => {
        if (data !== null) {
            return data
                .filter((x) => parseInt(x.ano) === selectedYear)
                .sort((a, b) => (parseInt(a.mes) - parseInt(b.month)) || a.nombre.localeCompare(b.nombre));
        }
    }, [data, selectedYear]);

    const handleOpenDialogUpdate = useCallback((payload) => {
        setForm((p) => ({ ...p, name: payload.nombre }));
        setForm((p) => ({ ...p, description: JSON.parse(payload.descripcion) }));
    ...
    const handlePostTimeline = useCallback(async () => {
        try {
            ...
            const requestBody = JSON.stringify({
                ...
                'people_id': personal ? String(people.id).trim() : '0',
                'is_general': personal ? 'NO' : 'SI',
    ...
    }, [form, token, people, personal, setForm, dispatch, setDialogCreate, handleGetTimeline]);
    ...
    const handlePutTimeline = useCallback(async () => {
        try {
            ...
            const requestBody = JSON.stringify({
                'id': String(selectedData?.id).trim(),
                'name': form.name.trim(),
                'description': JSON.stringify(form.description)
            });
    ...
    }, [form, token, selectedData, people, personal, setForm, dispatch, setDialogUpdate, handleGetTimeline]);
                            ...
                            {yearRecords.length === 0 ? (
                                ...
                                yearRecords.map((r) => (
                                    <TableRow key={r.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                        <TableCell>{months.find((x) => x.value === parseInt(r.mes)).label}</TableCell>
                                        <TableCell>{r.nombre}</TableCell>
                                        <TableCell>
                                            <ReactQuill
                                                value={JSON.parse(r.descripcion)}
                                                theme={null}
                                                readOnly={true}
                                            />
                                        </TableCell>
                    ...
                    {selectedData !== null && (
                        <Fragment>
                            ...
                            <Alert severity='info' sx={{ mb: 2 }}>
                                ...
                                {months.find((x) => x.value === parseInt(selectedData.mes)).label} del año {selectedData.ano}
                            </Alert>
                    ...
                    {selectedData !== null && (
                        <Fragment>
                            <Alert severity='warning' sx={{ mb: 2 }}>
                                <AlertTitle>Periodo de este item del Timeline</AlertTitle>
                                Esta seguro de eliminar el siguiente periodo en {months.find((x) => x.value === parseInt(selectedData.mes)).label} del año {selectedData.ano}
                            </Alert>
