...
const Index = ({ people, viewOnly }) => {
    ...
    const handleGetData = useCallback(async () => {
        try {
            ...
            if (responseJson.message !== null) {
                setTextoHebreo(responseJson.message.texto_hebreo);
                setTextoEspanol(responseJson.message.texto_espanol);
                setSimbolosUtilizados(JSON.parse(responseJson.message.simbolos_utilizados));
                setFechaColocacion(responseJson.message.fecha_colocacion);
                setMaterialConstruccion(JSON.parse(responseJson.message.material_construccion));
                setEstadoConservacionActual(JSON.parse(responseJson.message.estado_conservacion));
            }
        } catch (error) {
    ...
    }, [token, people, setLoading, dispatch, setTextoHebreo, setTextoEspanol, setSimbolosUtilizados, setFechaColocacion, setMaterialConstruccion, setEstadoConservacionActual]);
    ...
    const handlePutData = useCallback(async () => {
        try {
            ...
            const requestBody = JSON.stringify({
                ...
                'hebrew_text': textoHebreo.trim(),
                'spanish_text': textoEspanol.trim(),
                'date_colocation': fechaColocacion.trim(),
                'symbols_utilized': JSON.stringify(simbolosUtilizados),
                'conservation_state': JSON.stringify(estadoConservacionActual),
                'material_construction': JSON.stringify(materialConstruccion),
            });
    ...
    }, [token, people, textoHebreo, textoEspanol, fechaColocacion, simbolosUtilizados, estadoConservacionActual, materialConstruccion, dispatch, setLoading]);
