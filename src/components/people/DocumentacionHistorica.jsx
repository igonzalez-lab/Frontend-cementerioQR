...
const DocumentacionHistorica = ({ people, viewOnly }) => {
    ...
    const handlePostDocument = useCallback(async () => {
        try {
            ...
            const requestBody = JSON.stringify({
                ...
                'file_name': documentos[0].name,
                'original_language': idiomaOriginal,
                'file_base_64': documentos[0].base64,
                'preserve_state': estadoConservacion,
                'family_relationated': familiaRelacionada,
                'aproximated_date': fechaAproximadaDocumento,
                'digitalization_quality': calidadDigitalizacion,
                'content_description': JSON.stringify(descripcionContenido)
            });
    ...
    }, [token, page, rowsPerPage, filterValue, documentos, people, tipoDocumento, idiomaOriginal, familiaRelacionada, estadoConservacion, descripcionContenido, calidadDigitalizacion, fechaAproximadaDocumento, dispatch, handleGetData, handleGetDataCount, handleGetDataSearch, setLoading, setDialogCreate, setDocumentos, setTipoDocumento, setIdiomaOriginal, setFamiliaRelacionada, setEstadoConservacion, setDescripcionContenido, setCalidadDigitalizacion, setFechaAproximadaDocumento]);
    ...
    }, [token, page, rowsPerPage, filterValue, selectedData, documentos, people, tipoDocumento, idiomaOriginal, familiaRelacionada, estadoConservacion, descripcionContenido, calidadDigitalizacion, fechaAproximadaDocumento, dispatch, handleGetData, handleGetDataCount, handleGetDataSearch, setLoading, setDialogUpdate, setDocumentos, setTipoDocumento, setIdiomaOriginal, setFamiliaRelacionada, setEstadoConservacion, setDescripcionContenido, setCalidadDigitalizacion, setFechaAproximadaDocumento]);
    ...
    }, [token, page, rowsPerPage, filterValue, selectedData, dispatch, setDialogDelete]);
    ...
    const handleViewDocument = useCallback((base64, fileName) => {
        try {
            ...
            const blobUrl = `${import.meta.env.VITE_BACKEND_ADDRESS}\\${base64}`;
    ...
    const handleGetDocument = useCallback(async (id, nombre = '', soloBase64 = false) => {
        try {
            ...
            if (soloBase64) {
                return responseJson.message.ruta
            } else {
                handleViewDocument(responseJson.message.ruta, nombre);
            }
        }
        catch (error) {
            dispatch(SHOW_ERROR_MESSAGE(error.message));
    ...
    const handleOpenDialogUpdate = useCallback(async (payload) => {
        ...
        setTipoDocumento(payload.tipo_documento);
        setIdiomaOriginal(payload.idioma_original);
        setFamiliaRelacionada(payload.familia_relacionada);
        setEstadoConservacion(payload.estado_conservacion);
        setDescripcionContenido(JSON.parse(payload.descripcion_contenido));
        setCalidadDigitalizacion(payload.calidad_digitalizacion);
        setFechaAproximadaDocumento(payload.fecha_aproximada);

        const newFiles = [];
        const fileName = payload.nombre;
        const fileUrl = await handleGetDocument(payload.id, '', true);
    ...
    }, [handleGetDocument, setDocumentos, setDialogUpdate, setSelectedData, setTipoDocumento, setIdiomaOriginal, setFamiliaRelacionada, setEstadoConservacion, setDescripcionContenido, setCalidadDigitalizacion, setFechaAproximadaDocumento]);
                                                ...
                                                <TableBody>
                                                    {dataSearch.map((value, index) => (
                                                        <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 }, }}>
                                                            <TableCell align='center'>{items.filter(x => String(x.value) === String(value.tipo_documento))[0].item}</TableCell>
                                                            <TableCell align='center'>{value.idioma_original}</TableCell>
                                                            <TableCell align='center'>{value.estado_conservacion}</TableCell>
                                                            <TableCell align='center'>{digitalizacion.filter(x => String(x.value) === String(value.calidad_digitalizacion))[0].item}</TableCell>
                                                            <TableCell align='center'>{value.fecha_aproximada}</TableCell>
            ...
            <Dialog onClose={() => setDialogDelete(false)} open={dialogDelete} maxWidth='md' fullScreen={fullScreen}>
                ...
                <DialogContent>
                    {selectedData === null ? (
                        ...
                        <Fragment>
                            <Typography textAlign={'justify'}>¿Esta seguro de eliminar el documento <b>{items.filter(x => String(x.value) === String(selectedData.tipo_documento))[0].item} ({selectedData.nombre})</b> del sistema?, una vez eliminada deberá crearla nuevamente si desea restablecerla.</Typography>
                                    ...
                                    <Table>
                                        <TableBody>
                                            <TableRow>
                                                ...
                                                <TableCell>{items.filter(x => String(x.value) === String(selectedData.tipo_documento))[0].item}</TableCell>
                                            </TableRow>
                                            <TableRow>
                                                <TableCell sx={{ fontWeight: 900, bgcolor: grey['100'] }}>Idioma Original del Documento</TableCell>
                                                <TableCell>{selectedData.idioma_original}</TableCell>
                                            </TableRow>
                                            <TableRow>
                                                <TableCell sx={{ fontWeight: 900, bgcolor: grey['100'] }}>Estado de Conservación del Documento</TableCell>
                                                <TableCell>{selectedData.estado_conservacion}</TableCell>
                                            </TableRow>
                                            <TableRow>
                                                <TableCell sx={{ fontWeight: 900, bgcolor: grey['100'] }}>Calidad de digitalización del Documento</TableCell>
                                                <TableCell>{digitalizacion.filter(x => String(x.value) === String(selectedData.calidad_digitalizacion))[0].item}</TableCell>
                                            </TableRow>
                                            <TableRow>
                                                <TableCell sx={{ fontWeight: 900, bgcolor: grey['100'] }}>Fecha Aproximada del Documento</TableCell>
                                                <TableCell>{selectedData.fecha_aproximada}</TableCell>
                                            </TableRow>
                                            <TableRow>
                                                <TableCell sx={{ fontWeight: 900, bgcolor: grey['100'] }}>Familia Relacionada</TableCell>
                                                <TableCell>{selectedData.familia_relacionada}</TableCell>
                                            </TableRow>
                                            <TableRow sx={{ '&:last-child td, &:last-child th': { border: 0 }, }}>
                                                <TableCell sx={{ fontWeight: 900, bgcolor: grey['100'] }}>Documento</TableCell>
                                                <TableCell>
                                                    <Button variant='text' color='primary' size='small' startIcon={<ExitToApp />} onClick={() => handleGetDocument(selectedData.id, selectedData.nombre)}>
                                ...
                                <TableContainer>
                                    <Table>
                                        <TableBody>
                                            ...
                                            <TableRow sx={{ '&:last-child td, &:last-child th': { border: 0 }, }}>
                                                <ReactQuill
                                                    value={JSON.parse(selectedData.descripcion_contenido)}
                                                    theme={null}
                                                    className='rounded'
                                                    readOnly
                                                />
