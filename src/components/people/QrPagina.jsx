...
const QRImprimible = forwardRef(({ value, people, qrBase64Encrypted }, ref) => (
    <Box ref={ref} textAlign='center' width='fit-content' height='fit-content'>
        ...
        <Typography sx={{ mt: 1 }}>
            Escanea para ir ver a {people.nombre_espanol}
        </Typography>
    </Box>
));
            ...
            {people === null ? (
                ...
                <Box>
                    <Box textAlign='center' width='fit-content' height='fit-content'>
                        {qrCode ? (
                            <img src={`${import.meta.env.VITE_BACKEND_ADDRESS}\\${qrCode}`} alt='QR generado' width={200} style={{ display: 'block' }} />
                        ) : (
                            <QRCodeCanvas value={qrUrl} size={200} />
                        )}

                        <Typography sx={{ mt: 1 }}>Escanea para ir ver a {people.nombre_espanol}</Typography>
                    </Box>

                    <Box sx={{ position: 'fixed', left: -10000, top: 0 }}>
                        <QRImprimible ref={qrRef} value={qrUrl} people={people} qrBase64Encrypted={`${import.meta.env.VITE_BACKEND_ADDRESS}\\${qrCode}`} />
                    </Box>

                    <Box display='flex' alignItems='center' mt={2}>
                        <Button disableElevation color='primary' variant='contained' startIcon={<Download />} disabled={!qrCode || loading} onClick={() => downloadPNG(`${import.meta.env.VITE_BACKEND_ADDRESS}\\${qrCode}`)}>
