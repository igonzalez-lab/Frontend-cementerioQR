...
const HistoriaFamiliar = ({ people, viewOnly }) => {
    ...
    const handleGetData = useCallback(async () => {
        try {
            ...
            if (responseJson.message) {
                setPaisOrigen(responseJson.message.pais_origen);
                setCiudadOrigen(responseJson.message.ciudad_origen);
                setFechaLlegada(responseJson.message.fecha_llegada);
                setBarrioAsentamiento(responseJson.message.barrio_asentamiento);
                setComunidadReligiosa(responseJson.message.comunidad_religiosa);
                setHistoriaMigratoria(JSON.parse(responseJson.message.historia_migratoria));
                setTradicionesFamiliares(JSON.parse(responseJson.message.tradiciones_familiares));
            }
    ...
    }, [token, people, setLoading, dispatch, setPaisOrigen, setCiudadOrigen, setFechaLlegada, setBarrioAsentamiento, setComunidadReligiosa, setTradicionesFamiliares, setHistoriaMigratoria]);
    ...
    const handlePutData = useCallback(async () => {
        try {
            ...
            const requestBody = JSON.stringify({
                ...
                'origin_country': paisOrigen.trim(),
                'origin_city': ciudadOrigen.trim(),
                'arrive_date': fechaLlegada.trim(),
                'settlement_neighborhood': barrioAsentamiento.trim(),
                'religion_comunity': comunidadReligiosa.trim(),
                'migratory_history': JSON.stringify(historiaMigratoria),
                'familiar_traditions': JSON.stringify(tradicionesFamiliares),
            });
    ...
    }, [token, people, paisOrigen, ciudadOrigen, fechaLlegada, barrioAsentamiento, comunidadReligiosa, historiaMigratoria, tradicionesFamiliares, dispatch, setLoading]);
