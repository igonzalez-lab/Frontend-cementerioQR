...
const Index = () => {
    ...
    const handlePostPeople = useCallback(async () => {
        try {
            ...
            const requestBody = JSON.stringify({
                'spanish_name': spanishFullname.trim(),
                'yiddish_name': yiddishFullname.trim(),
                'hebrew_name': hebrewFullname.trim(),
                'gregorian_birth_date': gregorianBirthDate.trim(),
                'hebrew_birth_date': hebrewBirthDate.trim(),
                'gregorian_death_date': gregorianDeathDate.trim(),
                'hebrew_death_date': hebrewDeathDate.trim(),
                'country': countryBirth.trim(),
                'city': cityBirth.trim(),
                'region': regionBirth.trim(),
                'biography': JSON.stringify(deltaContent),
    ...
    const handlePutPeople = useCallback(async () => {
        try {
            ...
            const requestBody = JSON.stringify({
                'people_id': String(selectedData.id),
                'spanish_name': spanishFullname.trim(),
                'yiddish_name': yiddishFullname.trim(),
                'hebrew_name': hebrewFullname.trim(),
                'gregorian_birth_date': gregorianBirthDate.trim(),
    ...
    const handleOpenDialogUpdate = useCallback(async (payload) => {
        ...
        setSpanishFullname(payload.nombre_espanol);
        setYiddishFullname(payload.nombre_yiddish);
        setHebrewFullname(payload.nombre_hebreo);
        setHebrewBirthDate(payload.fecha_nacimiento_hebreo);
        setGregorianBirthDate(payload.fecha_nacimiento_gregoriano);
        setCityBirth(payload.ciudad);
        setRegionBirth(payload.region);
        setCountryBirth(payload.pais);
        setHebrewDeathDate(payload.fecha_muerte_hebreo);
        setGregorianDeathDate(payload.fecha_muerte_gregoriano);
        setDeltaContent(JSON.parse(payload.biografia));
        ...
        setFiles(newFiles);
    }, [setSelectedData, setDialogUpdate, setSpanishFullname, setYiddishFullname, setHebrewFullname, setHebrewBirthDate, setGregorianBirthDate, setCityBirth, setRegionBirth, setCountryBirth, setGregorianDeathDate, setHebrewDeathDate, setDeltaContent, setFiles, base64ToBlob, handleGetFilesById]);
    ...
    useEffect(() => {
        if (setPeopleRef.current === true) {
            if (data !== null) {
                if (String(jwtDecode(token).role) === '2') {
                    const ids = JSON.parse(jwtDecode(token).personas);
    ...
    }, [data, token, setPeopleRef, setData, jwtDecode]);
                                        ...
                                        <TableBody>
                                            {dataSearch.map((value, index) => (
                                                <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 }, }}>
                                                    <TableCell align='center'>{value.nombre_espanol}</TableCell>
                                                    <TableCell align='center'>{value.nombre_hebreo}</TableCell>
                                                    <TableCell align='center' style={{ borderRight: '1px solid rgba(0, 0, 0, 0.12)' }}>{value.nombre_yiddish}</TableCell>
                                                    <TableCell align='center'>{value.fecha_nacimiento_gregoriano}</TableCell>
                                                    <TableCell align='center' style={{ borderRight: '1px solid rgba(0, 0, 0, 0.12)' }}>{value.fecha_nacimiento_hebreo}</TableCell>
                                                    <TableCell align='center'>{value.pais}</TableCell>
                                                    <TableCell align='center' style={{ borderRight: '1px solid rgba(0, 0, 0, 0.12)' }}>{value.ciudad}</TableCell>
                                                    <TableCell align='center'>{value.fecha_muerte_gregoriano}</TableCell>
                                                    <TableCell align='center' style={{ borderRight: '1px solid rgba(0, 0, 0, 0.12)' }}>{value.fecha_muerte_hebreo}</TableCell>
            ...
            <Dialog onClose={() => setDialogDelete(false)} open={dialogDelete} maxWidth='md' fullScreen={fullScreen}>
                ...
                <DialogContent>
                    {selectedData === null ? (
                        ...
                        <Fragment>
                            <Typography textAlign={'justify'}>¿Esta seguro de eliminar a la persona <b>{selectedData.nombre_espanol}</b> del sistema?, una vez eliminada deberá crearla nuevamente si desea restablecerla.</Typography>
