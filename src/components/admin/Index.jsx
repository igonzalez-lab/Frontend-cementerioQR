...
const Index = () => {
    ...
    const handleOpenDialogUpdate = useCallback((row) => {
        ...
        setName(row.nombre);
        setSurname(row.apellido);
        setUsername(row.usuario);
        setRol(row.rol);
        setPersonas(JSON.parse(row.personas));
    ...
    const handlePutUser = useCallback(async () => {
        try {
            ...
            const requestBody = JSON.stringify({
                id: String(selectedUser.id).trim(),
                name: String(name).trim(),
                surname: String(surname).trim(),
                username: String(username).trim(),
                role: String(rol).trim(),
    ...
    const handleDeleteUser = useCallback(async () => {
        try {
            ...
            const requestBody = JSON.stringify({
                user_id: String(selectedUser.id).trim(),
            });
    ...
    const filteredUsers = filterUsersValue ? users.filter((row) =>
        ['nombre', 'apellido', 'usuario'].some((key) =>
            row[key] !== null && row[key].toLowerCase().includes(filterUsersValue.toLowerCase())
        )) : users;
                                        ...
                                        {filteredUsers
                                            ...
                                            .map((row) => (
                                                <TableRow key={row.id}>
                                                    ...
                                                    <TableCell align='center'>
                                                        {String(jwtDecode(token).id) !== String(row.id) && (
                                                            <Fragment>
            ...
            <Dialog open={dialogUpdate} fullScreen={fullScreen} maxWidth='sm' fullWidth onClose={() => setDialogUpdate(false)}>
                ...
                <DialogContent>
                    ...
                    {selectedUser === null ? (
                        ...
                        <Alert severity='info' sx={{ mb: 2 }}>Esta editando a {selectedUser.nombre} {selectedUser.apellido}</Alert>
                    )}
            ...
            <Dialog open={dialogDelete} fullScreen={fullScreen} maxWidth='sm' fullWidth onClose={() => setDialogDelete(false)}>
                ...
                <DialogContent>
                    {selectedUser === null ? (
                        ...
                        <Alert severity='info'>Esta seguro de eliminar al usuario <b>{selectedUser.nombre} {selectedUser.apellido}</b>?</Alert>
                    )}
                </DialogContent>
                {selectedUser !== null && (
                    <Fragment>
