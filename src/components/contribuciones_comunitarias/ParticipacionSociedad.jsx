...
const ParticipacionSociedad = ({ people, viewOnly }) => {
    ...
    const handleGetInstitutionalData = useCallback(async () => {
        try {
            ...
            if (responseJson.message !== null) {
                setInstitucionesComunitarias(JSON.parse(responseJson.message.instituciones_fundadas));
                setCargosDirectivos(JSON.parse(responseJson.message.cargos_directivos));
                setPeriodoParticipacion(JSON.parse(responseJson.message.periodo_participacion));
                setLogrosReconocimientos(JSON.parse(responseJson.message.logros_reconocimientos));
                setProyectosImportantes(JSON.parse(responseJson.message.proyectos_importantes));
                setLegadoInstitucional(JSON.parse(responseJson.message.legado_institucional));
            }
        }
        catch (error) {
    ...
    }, [people, token, dispatch, setCargando, setInstitucionesComunitarias, setCargosDirectivos, setPeriodoParticipacion, setLogrosReconocimientos, setProyectosImportantes, setLegadoInstitucional]);
    ...
    const handlePutInstitutionalData = useCallback(async () => {
        try {
            ...
            const requestBody = JSON.stringify({
                'people_id': String(people.id),
                'participation_type': "NO",
                'instituciones_fundadas': JSON.stringify(institucionesComunitarias),
                'cargos_directivos': JSON.stringify(cargosDirectivos),
                'periodo_participacion': JSON.stringify(periodoParticipacion),
                'logros_reconocimientos': JSON.stringify(logrosReconocimientos),
                'proyectos_importantes': JSON.stringify(proyectosImportantes),
                'legado_institucional': JSON.stringify(legadoInstitucional)
            });
    ...
    }, [people, token, institucionesComunitarias, cargosDirectivos, periodoParticipacion, logrosReconocimientos, proyectosImportantes, legadoInstitucional, dispatch, setCargando]);
                    ...
                    <Box ref={containerRef} component={Paper} elevation={0} variant='outlined' display={'flex'} flexDirection={'column'} p={2}>
                        ...
                        <FormControl fullWidth>
                            <InputLabel shrink sx={{ bgcolor: common['white'], px: 0.8, color: focusedLegadoInstitucional ? 'primary.main' : 'text.secondary', transition: 'color 0s', display: 'flex', alignItems: 'center' }}>
                                <Emergency color='error' fontSize='small' sx={{ mr: 0.2 }} />
                                Legado institucional / wikipedia
                            </InputLabel>
...
export default ParticipacionSociedad;
