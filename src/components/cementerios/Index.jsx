...
import { Box, Breadcrumbs, Typography, useMediaQuery, useTheme, Link as LinkMUI, Button, TextField, Paper, CircularProgress, TableContainer, Table, TableHead, TableRow, TableCell, Pagination, TableBody, Dialog, DialogTitle, DialogContent, DialogActions, Tooltip, Collapse, Alert, IconButton, ListItemIcon, ListItemText, Menu, MenuList, MenuItem } from '@mui/material';
import { Close, Edit, FilterList, FilterListOff, Home, PersonAdd, Refresh, Replay, Warning, Emergency, ArrowBack, Filter1, Filter2, Filter3, Filter4, Filter5, Filter6, Filter7, Filter8, Filter9, FormatListBulleted, ExitToApp } from '@mui/icons-material';
...
const Index = () => {
    ...
    }, [page, token, rowsPerPage, filterValue, selectedData, name, direction, dispatch, setDialogUpdate, handleGetData, handleGetDataSearch]);
    ...
    const handleOpenDialogUpdate = useCallback(async (payload) => {
        ...
        setName(payload.nombre);
        setDirection(payload.direccion);
    }, [setDialogUpdate, setSelectedData, setName, setDirection]);
    ...
    return (
        <Box maxWidth={mobileScreen ? '87vw' : '100%'}>
                    ...
                    {dataSearchError ? (
                        <Box component={Paper} elevation={0} variant='outlined' display={'flex'} flexDirection={'column'} justifyContent={'center'} p={2}>
                            <Box display={'flex'} alignItems={'center'} mb={1}>
                                ...
                                <Typography ml={1}>Error al obtener los cementerios: {dataSearchErrorMessage}</Typography>
                            </Box>
                                        ...
                                        <TableBody>
                                            {dataSearch.map((value, index) => (
                                                <TableRow key={index} sx={{ '&:last-child td, &:last-child th': { border: 0 }, }}>
                                                    ...
                                                    <TableCell align='center'>{value.direccion}</TableCell>
                                                    <TableCell align='center'>
                                                        <Button variant='text' color='primary' startIcon={<ExitToApp />} LinkComponent={'a'} href={`https://www.google.com/maps/search/${value.nombre}`} target='_blank'>
            ...
            <Dialog onClose={() => setDialogDelete(false)} open={dialogDelete} maxWidth='md' fullScreen={fullScreen}>
                ...
                <DialogContent>
                    {selectedData ? (
                        <Alert severity='warning'>Esta seguro de eliminar el cementerio <b>{selectedData.nombre}</b> ubicado en <b>{selectedData.direccion}</b></Alert>
                    ) : (
                        <Alert severity='info'>Seleccione un cementerio para continuar</Alert>
                    )}
                </DialogContent>
